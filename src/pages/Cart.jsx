					...
					cartData.map((item, index) => {
                     ...
                     <div key={index} className="grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_2fr] gap-6 text-gray-700 border-b border-t mb-3 px-3">
                        <div className="flex sm:gap-20 py-5 items-center justify-between w-full  ">
                           ...
                           <input type="number" onChange={(e)=> e.target.value === '' || e.target.value === '0'?null:updateQuantity(item.id,item.size,Number(e.target.value))} className='border max-w-10 sm:max-w-20 sm:px-2 px-1 py-1 max-h-7' defaultValue={item.quantity} min={1}/>
                           <i onClick={()=>updateQuantity(item.id,item.size,0)} className="ri-delete-bin-6-line cursor-pointer"></i>
                        </div>
                     </div>
                  )
					})
